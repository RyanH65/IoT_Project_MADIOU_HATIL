...
// With Battery Chassis (Grove UART connector at the left side of the USB-C
// connector)
...
static int
at_send_check_response(char* p_ack, int timeout_ms, char* p_cmd, ...)
{
  int ch;
  int num = 0;
  int index = 0;
...
loopE5(void)
{
  if (is_exist) {
    ...
    if (is_join) {
      ...
      int NO2 = sensors[0].last_val;
      int CO = sensors[1].last_val;
      int C2H5CH = sensors[2].last_val;
      int VOC = sensors[3].last_val;
